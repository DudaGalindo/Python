...
class general:
    ...
    def Kg_Fg(n_nos_tot,xCC,valor_CC,Kg,Fg):
...
class Barra:
    ...
    def Global(n_el,n_nos_tot,n_nos_el,x,integralAdx,E,conec,Fc,f,Kg,Fg):
    ...
    def deslocamento(n_nos_tot,n_el,n_nos_el,x,integralAdx,E,conec,Fc,f,xCC,valor_CC):
        ...
        Kg,Fg = Barra.Global(n_el,n_nos_tot,n_nos_el,x,integralAdx,E,conec,Fc,f,Kg,Fg)
...
class Viga:
    ...
    def Elem(E,I,he,cf,q,n_el):
        ...
        return Kel, Fel
    def Global(E,I,cf,Fc,x,Kg,Fg,n_el,n_nos_el,conec,q):

        for el in range(0,n_el):
            he = abs(x[el] - x[el+1])
            Kel, Fel = Viga.Elem(E,I,he,cf,q,n_el)
            for i in range(0,n_nos_el[el]):
                ig = conec[el,i] - 1
                Fg[ig] = Fg[ig] + Fel[i]
                for j in range(0,n_nos_el[el]):
    ...
    def deslocamento(ngl_tot,n_nos_tot,n_el,n_nos_el,x,I,E,Fc,q,xCC,valor_CC,cf,ngl_no):
        conec = np.zeros((ngl_tot,2))
        for i in range(0,ngl_tot):
            conec[i,0] = i
            conec[i,1] = i+1
        Kg,Fg = general.initialize(n_nos_tot)
        Kg,Fg = Viga.Global(E,I,cf,Fc,x,Kg,Fg,n_el,n_nos_el,conec,q)
        Kg,Fg = general.Kg_Fg(n_nos_tot,xCC,valor_CC,Kg,Fg)
        Kg_inv = np.linalg.inv(Kg)
